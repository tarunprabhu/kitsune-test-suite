...
void cpy(kitsune::mobile_ptr<float> dst_p,
         const kitsune::mobile_ptr<float> src_p, int n) {
...
static void
initialize_variables(int nelr, kitsune::mobile_ptr<float> variables_p,
                     const kitsune::mobile_ptr<float> ff_variable_p) {
  // FIXME: Kokkos cannot deal with the mobile_ptr type for ... reasons.
  // We could try to find a way to make that type Kokkos-friendly, or just wait
  // until the [mobile_ptr] attribute is implemented which should make
  // Kokkos happy.
...
void compute_step_factor(int nelr, const kitsune::mobile_ptr<float> variables_p,
                         const kitsune::mobile_ptr<float> areas_p,
                         kitsune::mobile_ptr<float> step_factors_p) {
  // FIXME: Kokkos cannot deal with the mobile_ptr type for ... reasons.
  // We could try to find a way to make that type Kokkos-friendly, or just wait
  // until the [mobile_ptr] attribute is implemented which should make
  // Kokkos happy.
...
void compute_flux(
    int nelr, const kitsune::mobile_ptr<int> elements_surrounding_elements_p,
    const kitsune::mobile_ptr<float> normals_p,
    const kitsune::mobile_ptr<float> variables_p,
    kitsune::mobile_ptr<float> fluxes_p,
    const kitsune::mobile_ptr<float> ff_variable_p,
...
void time_step(int j, int nelr, kitsune::mobile_ptr<float> old_variables_p,
               kitsune::mobile_ptr<float> variables_p,
               kitsune::mobile_ptr<float> step_factors_p,
               kitsune::mobile_ptr<float> fluxes_p) {
  // FIXME: Kokkos cannot deal with the mobile_ptr type for ... reasons.
  // We could try to find a way to make that type Kokkos-friendly, or just wait
