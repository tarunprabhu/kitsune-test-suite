...
void parallel_copy(mobile_ptr<T> dst, const mobile_ptr<T> src, int N) {
  // clang-format off
  // FIXME: Simply using dst and src in the Kokkos lambda does not compile. The
  // Kokkos are, unsurprisingly, utterly useless. For now, this works, so we
  // will leave it like this. This should eventually be changed to use
  // attributed pointers instead, which should work like the regular pointers
  // and, hopefully, keep Kokkos happy.
  T* d = dst.get();
  const T* s = src.get();
