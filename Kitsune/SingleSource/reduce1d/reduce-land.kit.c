// Simple reduction test with loop of depth 1 (operator: logical AND)
...
static int test(bool *[[kitsune::mobile]] arr, long n, bool expected, bool init,
  ...
  forall (long i = 0; i < n; ++i) {
    __kitsune_reduce(&r, KIT_LAND, arr[i]);
  }
  // clang-format on
...
static int testFalse(bool *[[kitsune::mobile]] arr, long n) {
  for (long i = 0; i < n; ++i)
    arr[i] = false;
  return test(arr, n, false, true, 2);
}
...
static int testSome(bool *[[kitsune::mobile]] arr, long n) {
  for (long i = 0; i < n; ++i)
    arr[i] = true;
  arr[n / 2] = false;
  return test(arr, n, false, true, 3);
}
